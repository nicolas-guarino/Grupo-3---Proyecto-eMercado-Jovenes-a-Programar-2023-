  ...
  async function getCartItems() {
    try {
      ...
      <tr>
        ...
        <td><input type="number" id="cartCount" value="${1}" class="cartCant"></td>
      ...
      const subTotal = document.getElementById(`cartSub${cart[0].id}`);
      ...
      cant.addEventListener('input', function () {
        ...
        const unitCost = cart[0].unitCost;
        subTotal.textContent = `${cart[0].currency} ${count * unitCost}`;
        cant.value = count;
      });
    } catch (error) {
